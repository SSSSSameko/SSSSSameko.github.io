...
try {
  ...
  assert.equal(await page.getByText('版本 3.4.1 · by.sameko', { exact: true }).first().isVisible(), true);
  ...
  await updates.waitFor({ state: 'visible' });
    assert.equal(await updates.getByText('更新日期：2026 年 9 月 12 日', { exact: true }).isVisible(), true);
    assert.equal(await updates.getByText('版本 3.4.1', { exact: true }).isVisible(), true);

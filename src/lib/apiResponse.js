...
function responseTimeoutError(timeoutMs) {
  const error = new Error(`响应正文读取超时（${Math.ceil(timeoutMs / 1000)} 秒）`);
...
function responseTooLargeError(maxBytes) {
  const error = new Error(`响应正文超过 ${Math.ceil(maxBytes / 1024 / 1024)} MB 限制`);
...
export async function readResponseTextWithin(response, options = {}) {
  ...
  timer = setTimeout(() => {
    const error = responseTimeoutError(timeoutMs);
    stop(error);
    cancelBody(response, reader, error);
  }, timeoutMs);
  if (signal) {
  ...
  try {
    if (!reader) {
      ...
      if (byteLength(text) > maxBytes) throw responseTooLargeError(maxBytes);
      return text;
    }
...
export function isResponseBodyTimeout(error) {
  return error?.code === 'RESPONSE_BODY_TIMEOUT' || error?.name === 'TimeoutError';
}

...
window.WEIBO_DRAW_LEGAL = window.WEIBO_DRAW_LEGAL || {
  ...
  serverRegion: '中国天津', // 选填，但必须与真实机房一致
  // 下面按“GitHub Pages 静态前端 + 中国天津自建后端”的部署方式撰写；换成别的托管方式时请改写。
  dataRecipientDisclosure: '前端静态页面由 GitHub Pages 托管，只分发 HTML、CSS 和 JavaScript 文件，不接收候选名单、开奖结果或登录态；后端为运营者自行维护的服务器，位于中国天津，候选数据、开奖记录和服务器登录资料只保存在该服务器上。除为完成载入而向微博接口发出的必要请求外，不向其他第三方提供数据，也不委托第三方处理。',
  jurisdictionDisclosure: '本站后端服务器位于中国天津。因本站产生的争议，应先通过页面公布的联系邮箱协商解决；协商不成的，依法向有管辖权的争议解决机构或法院提出。',

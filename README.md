...
版本记录统一维护在 `docs/CHANGELOG.md`，新版本追加在文件末尾；站内“更新日志”和后端版本信息均由该文件生成。格式为「## 版本号 · 发布日期」，下接一句话标题和若干短条目，一律从使用者视角描述能感知到的变化，不写内部实现、文件名、函数名或安全细节。每次发布必须同步提升 `package.json` 版本号，并让更新日志最后一条与包版本一致；构建和服务启动都会校验，不一致时直接失败。
